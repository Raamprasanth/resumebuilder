...
export const templateStyles: Record<string, { font: string; primaryColor: string; headerBorder: string }> = {
...
export function generateHtmlResumeString(input: GenerateResumeInput): string {
  ...
  const experiencesHtml = experiences.map(exp => `
    <div style="margin-bottom: 20px;">
        <div style="display: flex; justify-content: space-between;">
            <h3 style="margin: 0; font-size: 16px; font-weight: bold; color: var(--primary-color);">${exp.jobTitle}</h3>
  ...
  const educationHtml = education.map(edu => `
    <div style="margin-bottom: 15px;">
        <div style="display: flex; justify-content: space-between;">
            <h3 style="margin: 0; font-size: 15px; font-weight: bold; color: var(--primary-color);">${edu.degree}</h3>
  ...
  const projectsHtml = projects && projects.length > 0 ? projects.map(proj => `
    <div style="margin-bottom: 15px;">
        <div style="display: flex; justify-content: space-between;">
            <h3 style="margin: 0; font-size: 15px; font-weight: bold; color: var(--primary-color);">${proj.name}</h3>
...
<style>
  :root {
    ...
    --header-border: ${style.headerBorder};
    --font-family: ${style.font};
  }
</style>
<div id="resume-container" contenteditable="true" style="font-family: var(--font-family); line-height: 1.5; color: #333; background-color: #fff; width: 210mm; min-height: 297mm; padding: 25mm 20mm; box-sizing: border-box; outline: none;">
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 25px;">
        <div style="flex: 1;">
            <h1 style="font-size: 32px; font-weight: bold; margin: 0 0 10px 0; color: var(--primary-color);">${fullName}</h1>
            <p style="font-size: 13px; margin: 0; color: #555;">${email} | ${phone}</p>
        </div>
        ${photoDataUri ? `
        <div style="width: 100px; height: 100px; border-radius: 50%; overflow: hidden; border: 2px solid var(--primary-color); flex-shrink: 0; margin-left: 20px;">
            <img src="${photoDataUri}" alt="Profile Photo" style="width: 100%; height: 100%; object-fit: cover;" />
        </div>
        ` : ''}
    </div>

    <h2 style="font-size: 14px; font-weight: bold; text-transform: uppercase; margin-bottom: 10px; color: var(--primary-color); border-bottom: 1px solid var(--header-border); padding-bottom: 5px;">Summary</h2>
    <p style="font-size: 13px; margin-bottom: 25px; color: #444;">${summary}</p>

    <h2 style="font-size: 14px; font-weight: bold; text-transform: uppercase; margin-bottom: 15px; color: var(--primary-color); border-bottom: 1px solid var(--header-border); padding-bottom: 5px;">Experience</h2>
    ${experiencesHtml}

    <h2 style="font-size: 14px; font-weight: bold; text-transform: uppercase; margin-bottom: 15px; margin-top: 25px; color: var(--primary-color); border-bottom: 1px solid var(--header-border); padding-bottom: 5px;">Education</h2>
    ${educationHtml}

    ${projectsHtml ? `
    <h2 style="font-size: 14px; font-weight: bold; text-transform: uppercase; margin-bottom: 15px; margin-top: 25px; color: var(--primary-color); border-bottom: 1px solid var(--header-border); padding-bottom: 5px;">Projects</h2>
    ${projectsHtml}
    ` : ''}

    <h2 style="font-size: 14px; font-weight: bold; text-transform: uppercase; margin-bottom: 15px; margin-top: 25px; color: var(--primary-color); border-bottom: 1px solid var(--header-border); padding-bottom: 5px;">Skills</h2>
