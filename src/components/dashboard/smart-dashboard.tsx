...
import { generateJobRecommendations, JobRecommendationOutput } from '@/ai/flows/job-recommendation';
...
export function SmartDashboard() {
  ...
  const [searchResult, setSearchResult] = useState<JobRecommendationOutput | null>(null);
    ...
    try {
        ...
        if (typeof window !== 'undefined' && window.sessionStorage) {
         ...
         toast({
            ...
            title: 'Search Failed',
            description:
            'There was an error finding jobs. Please try again.',
         });
    } finally {
  ...
  const recommendations = scanResult?.recommendations || searchResult?.recommendations || [];
  ...
  return (
    <div className="space-y-6">
       <Card>
        ...
        <CardContent>
             <Form {...searchForm}>
                <form onSubmit={searchForm.handleSubmit(onSearchSubmit)} className="grid sm:grid-cols-3 gap-4 items-end">
                    <FormField
          ...
          <AccordionContent>
             <Card className="border-none shadow-none">
              <CardDescription className="px-6 pb-4">
                Upload your resume to automatically get personalized job recommendations and see how you match.
              </CardDescription>
              <CardContent>
                <Form {...scanForm}>
      ...
      {(isLoading || isSearching) && (
        <div className="mt-6 flex flex-col items-center justify-center space-y-4 min-h-[200px]">
          ...
          <p className="text-muted-foreground">
            {isLoading ? 'Analyzing resume and finding jobs...' : 'Searching for jobs...'}
          </p>
        </div>
      )}
        ...
        <div className="mt-6">
            <div className="mb-6">
                ...
                {scanResult ? (
                    ...
                    <p className="text-muted-foreground">
                        Showing results for <span className="font-semibold text-primary">{searchForm.getValues('jobTitle')}</span> in <span className="font-semibold text-primary">{searchForm.getValues('location')}</span>.
                    </p>
                )}
            </div>
              ...
              {recommendations.map((job) => (
                <Card key={job.id} className="flex flex-col text-center">
                    <CardContent className="flex-grow flex flex-col items-center justify-center p-6">
                        ...
                        <p className="text-sm text-muted-foreground mt-3 flex-grow">{job.title}</p>
                    </CardContent>
